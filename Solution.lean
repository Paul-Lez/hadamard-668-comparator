...
This file applies the Wallis–Whiteman construction to cyclic sign sequences from the example
announced by Levent Alpöge, Philippe Voinov, Saul Reynolds-Haertle, and Claude. The stored
sequences differ from the posted representatives by operations that preserve the required
correlations.

## Construction

Let $s_1,s_2,s_3,s_4 : \mathbb Z/166\mathbb Z \to \{\pm1\}$ be the four sign sequences stored in
`sBits`. For a sequence $x$, define $S(x)_{ij}=x_{i-j}$, and let $R$ be the permutation matrix
for $i\mapsto-i$. Write $A=S(s_1), B=S(s_2), C=S(s_3), D=S(s_4)$. These matrices form the
$664\times664$ core
...
Let $X,Y,Z$ be the fixed $4\times4$ matrices defined below. Repeating each column of $Y$ 166
times gives $\widetilde Y$ of size $4\times664$; repeating each row of $Z$ 166 times gives
$\widetilde Z$ of size $664\times4$. The final matrix is
...
$$
\sum_i s_1(i)=2,\qquad \sum_i s_2(i)=\sum_i s_3(i)=\sum_i s_4(i)=0,
$$

and

$$
P(t)=\sum_{q=1}^4\sum_i s_q(i)s_q(i+t)
...
The same argument works for sequences of any length $v$ with sums $(2,0,0,0)$ and combined
periodic autocorrelation $4v$ at zero and $-4$ elsewhere, producing a matrix of order $4(v+1)$.
Equivalently, the $+1$ supports here form a difference family of type $H_4^*$ with parameters
$(166;84,83,83,83;166)$. Complementing the first block gives the conventional parameters
$(166;83,83,83,82;164)$.
...
- [A new family of Hadamard matrices of order $4(2q^2+1)$](https://arxiv.org/abs/1907.02623)
  by *Ka Hin Leung, Koji Momihara, and Qing Xiang* (2019)
-/
